...
def test_mem_compact_dry_run_and_write(tmp_path):
    ...
    (src / "snapshot-aaaa.full.json").write_text(
        '{"schema":1,"mode":"full","etag_to":"aaaa","codec":"none","level":0}\n{"ok":true}'
    )
    # dry-run
    r = subprocess.run(
        [sys.executable, SCRIPT, "--in", str(src), "--out", str(dst), "--dry-run"],
        capture_output=True,
        text=True,
        check=False,
    )
    ...
    dst2 = tmp_path / "dst2"
    r2 = subprocess.run(
        [sys.executable, SCRIPT, "--in", str(src), "--out", str(dst2)],
        capture_output=True,
        text=True,
        check=False,
    )
