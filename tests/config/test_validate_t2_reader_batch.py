...
def test_cli_validator_rejects_invalid_reader_batch_and_embed_root(tmp_path: Path):
    ...
    assert "t2.reader_batch" in res.stdout or "t2.embed_root" in res.stdout, f"STDOUT:\n{res.stdout}\nSTDERR:\n{res.stderr}"
...
def test_cli_validator_warns_partitions_with_perf_disabled(tmp_path: Path):
    ...
    assert res.returncode == 0
    assert "W[perf.t2.reader]" in res.stdout, f"Expected partitions+perf.disabled warning. STDOUT:\n{res.stdout}\nSTDERR:\n{res.stderr}"