...
GOLDEN = Path("tests/reflection/goldens/enabled_llm/bench_llm.json")
CFG = "examples/reflection/llm_fixture.yaml"
...
@pytest.mark.skipif(not FIXTURES.exists(), reason="LLM fixtures missing; add tests/fixtures/reflection_llm.jsonl or skip this test.")
...
def test_llm_bench_matches_golden() -> None:
    got = _run_bench(CFG)
    want = json.loads(GOLDEN.read_text(encoding="utf-8"))

    assert got == want, (
        "LLM fixtures reflection bench output diverged from golden.\n"
        f"GOT:  {json.dumps(got, sort_keys=True)}\n"
        f"WANT: {json.dumps(want, sort_keys=True)}\n"
        "To refresh golden deterministically (only if fixtures file exists):\n"
        "  CI=true CLEMATIS_NETWORK_BAN=1 python -m clematis.scripts.bench_reflection "
        "-c examples/reflection/llm_fixture.yaml | tee tests/reflection/goldens/enabled_llm/bench_llm.json\n"
    )
