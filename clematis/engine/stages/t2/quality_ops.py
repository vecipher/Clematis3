...
def _bm25_scores(
    query: str, items: List[Dict[str, Any]], k1: float, b: float, stopset, qcfg: Dict[str, Any]
...
def fuse(
    query: str, items: List[Dict[str, Any]], *, cfg: Dict[str, Any]
    ...
    sem_ids = [
        it["id"]
        for it in sorted(items, key=lambda d: (-float(d.get("score", 0.0)), str(_id_or(d))))
    ]
    lex_ids = [
        it["id"]
        for it in sorted(
            items, key=lambda d: (-float(lex_scores.get(d["id"], 0.0)), str(_id_or(d)))
        )
    ]
