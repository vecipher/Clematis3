...
function pickGraph(bundle: RunBundle): { nodes: Node[]; edges: Edge[] } {
  ...
  const s = (bundle.snapshots as any[] | undefined)?.[0] ?? {};
  const nodes: Node[] =
    Array.isArray(s.nodes)
      ? s.nodes.map((n: any) => ({ id: String(n.id ?? n.label ?? n.name ?? "") })).filter((n: Node) => !!n.id)
      : Array.isArray((s.summary || {}).nodes)
      ? (s.summary.nodes as any[]).map((n: any) => ({ id: String(n.id ?? n.label ?? n.name ?? "") })).filter((n: Node) => !!n.id)
      : [];
  const edges: Edge[] =
    Array.isArray(s.edges)
      ? s.edges.map((e: any) => ({ src: String(e.src ?? e.from ?? ""), dst: String(e.dst ?? e.to ?? "") }))
              .filter((e: Edge) => !!(e.src && e.dst))
      : Array.isArray((s.summary || {}).edges)
      ? (s.summary.edges as any[]).map((e: any) => ({ src: String(e.src ?? e.from ?? ""), dst: String(e.dst ?? e.to ?? "") }))
              .filter((e: Edge) => !!(e.src && e.dst))
      : [];
  return { nodes, edges };
}

export function renderGraph(container: HTMLElement, bundle: RunBundle) {
  ...
  const sorted = nodes
    .map((n: Node, i: number) => ({ n, i }))
    .sort((a: { n: Node; i: number }, b: { n: Node; i: number }) =>
      (a.n.id < b.n.id ? -1 : a.n.id > b.n.id ? 1 : a.i - b.i)
    )
    .map((x: { n: Node; i: number }) => x.n);
