...
export function renderLogs(container: HTMLElement, bundle: RunBundle, showPerf: boolean, selected: string | null) {
  clear(container);
  ...
  for (const k of keys) select.append(el("option", { value: k, selected: selected === k }, [k]));
