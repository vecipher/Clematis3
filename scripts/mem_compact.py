...
def run(indir: str, outdir: str, dtype: str|None, compression: str, level: int, delta: bool, dry_run: bool) -> int:
    src = Path(indir); dst = Path(outdir)
    if not src.is_dir():
        print(f"input dir not found: {src}", file=sys.stderr); return 2
    _ensure_empty_dir(dst)
    # Only operate on full snapshots; safe and deterministic
    inputs = sorted(list(src.glob("snapshot-*.full.json")) + list(src.glob("snapshot-*.full.json.zst")))
    plan = []
    for ip in inputs:
...
if __name__ == "__main__":
    ...
    ap.add_argument("--in", dest="indir", required=True, help="input snapshots dir (reads full snapshots)")
    ap.add_argument("--out", dest="outdir", required=True, help="output dir (must be empty or new)")
    ap.add_argument("--dtype", choices=["fp32","fp16"], help="rewrite header dtype (metadata only)")
    ...
    args = ap.parse_args()
    sys.exit(run(args.indir, args.outdir, args.dtype, args.compression, args.level, args.delta, args.dry_run))