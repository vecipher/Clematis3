...
def run(root: str, snapshots_dir: str, fmt: str, verbose: bool) -> int:
    ...
    files = []
    if sdir.is_dir():
        for p in sorted(sdir.glob("snapshot-*.*.json")) + sorted(sdir.glob("snapshot-*.*.json.zst")):
    ...
    out = {"root": str(Path(root).resolve()), "snapshots_dir": str(sdir.resolve()), "files": files, "summary": summary}
    if fmt == "json":
        print(json.dumps(out, sort_keys=True))
    else:
        print(f"{'NAME':60}  KIND   CMP  LVL  BYTES       ETAG_TO      DELTA_OF")
        for f in files:
            print(f"{f['name'][:60]:60}  {f['kind']:<5}  {int(f['compressed'])}    {f['level']:>2}  {f['size_bytes']:>10}  {str(f['etag_to'])[:8]:>8}  {str(f.get('delta_of'))[:8]:>8}")
